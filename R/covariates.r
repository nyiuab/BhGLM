...
covariates0 <- function (covars, rescale = TRUE, fill.missing = FALSE)
{
  covars <- as.data.frame(covars)
  n.vars <- ncol(covars)
  if(is.null(colnames(covars))) {
    warning("Some variables have no names", call. = FALSE)
    colnames(covars) <- paste("V", 1:n.vars, sep = "")
  }
  var.names <- colnames(covars)
  covars.new <- NULL
  for (i in 1:n.vars) {
    x <- x.new <- covars[, i]
    ...
    if (is.factor(x)){
      ...
      mf <- model.frame(f, na.action = na.pass)
      x.new <- model.matrix(f, mf)[, -1, drop = FALSE]
      colnames(x.new) <- paste(var.names[i], levels(x)[-1], sep = ".")
      if (ncol(x.new) > 1) {
        rsums <- rowSums(x.new)
        if (all(rsums[!is.na(rsums)] == 1)) x.new <- x.new[, -1, drop = FALSE]
      }
    }
    if (is.numeric(x) & rescale) {
      x.obs <- x[!is.na(x)]
      if (length(unique(x.obs)) == 2)
        x <- (x - min(x.obs))/(max(x.obs) - min(x.obs))
      else
        x <- (x - mean(x.obs)) / (2 * sd(x.obs))
      x.new <- as.matrix(x)
      colnames(x.new) <- var.names[i]
    }
    covars.new <- cbind(covars.new, x.new)
    if (fill.missing) {
      func <- function(w){
        na.index <- which(is.na(w))
        w[na.index] <- mean(w, na.rm = TRUE)
      ...
      covars.new <- apply(covars.new, 2, func)
    }
  }
  w <- apply(covars.new, 2, var, na.rm = TRUE)
  covars.new <- covars.new[, which(w != 0), drop = FALSE]
  data.frame(covars.new)
}
